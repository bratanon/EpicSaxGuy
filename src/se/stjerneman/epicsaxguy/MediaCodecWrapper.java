...
public class MediaCodecWrapper {
    ...
    public boolean writeSample(final ByteBuffer input,
        ...
        if (size > 0 && !mAvailableInputBuffers.isEmpty()) {
            ...
            // Submit the buffer to the codec for decoding. The
            // presentationTimeUs
        ...
        while ((index = mDecoder.dequeueOutputBuffer(info, 0)) != MediaCodec.INFO_TRY_AGAIN_LATER) {
            switch (index) {
                ...
                default:
                    ...
                    // &
                    // INFO_OUTPUT_BUFFERS_CHANGED i.e all the other possible
                    // return codes but
                    // asserting index value anyways for future-proofing the
                    // code.
                    if (index >= 0) {
